...
with source_data as (
    ...
    select
        ...
        flattened.value:campaign_id::string     as campaign_id,
        ext.file_name                           as _source_file,
        ext.file_row_number                     as _source_row_number,
        current_timestamp()                     as _loaded_at,
        '{{ invocation_id }}'                   as _batch_id

    from {{ source('bronze_ext', 'ext_campaigns') }} as ext,
         lateral flatten(input => ext.value:campaigns_data) as flattened
