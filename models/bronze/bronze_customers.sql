...
with source_data as (
    ...
    select
        ...
        flattened.value:customer_id::string    as customer_id,
        ext.file_name                          as _source_file,
        ext.file_row_number                    as _source_row_number,
        current_timestamp()                    as _loaded_at,
        '{{ invocation_id }}'                  as _batch_id

    from {{ source('bronze_ext', 'ext_customers') }} as ext,
         lateral flatten(input => ext.value:customers_data) as flattened
