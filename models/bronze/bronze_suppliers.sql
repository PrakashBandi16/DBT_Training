...
with source_data as (
    ...
    select
        ...
        flattened.value:supplier_id::string     as supplier_id,
        ext.file_name                           as _source_file,
        ext.file_row_number                     as _source_row_number,
        current_timestamp()                     as _loaded_at,
        '{{ invocation_id }}'                   as _batch_id

    from {{ source('bronze_ext', 'ext_suppliers') }} as ext,
         lateral flatten(input => ext.value:suppliers_data) as flattened
